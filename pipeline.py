...
LUA_SHA1 = get_hash(os.path.join(CWD, 'nujij.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "nujij-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("nujij-item: %(item_name)s"),
        ]
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="nujij"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
